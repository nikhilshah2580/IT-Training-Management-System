...
const InstructorModulePage = ({
  ...
  const { data: courseData } = useQuery({
    queryKey: ["instructor-courses-options"],
    queryFn: () => getMyCourses({ limit: 100 }),
  });

  const { data: enrollmentData } = useQuery({
    queryKey: ["instructor-enrollments-options"],
    queryFn: () => getInstructorEnrollments({ limit: 100 }),
  });
  ...
  const students = Array.from(
    new Map(
      enrollments
        .filter((enrollment) => enrollment?.student?._id)
          ...
          <div className="mb-6 flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Fill in the fields below to manage module options.
              </p>
            </div>
            {editing && (
              <button
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {fields.map((field) => {
              ...
              if (field.type === "select") {
                const optionRows = field.optionsKey
                  ...
                  <div key={field.name} className="flex flex-col gap-1.5">
                    ...
                    >
                      ...
                      {optionRows.map((option) => (
                        ...
                        >
                          {option.title || option.fullName || option.label}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  >
                    ...
                    <div className="relative flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-6 transition hover:bg-slate-100/50">
                      ...
                      <p className="text-xs font-medium text-slate-600">
                        Click or drag to upload media asset
                      </p>
                      <p className="text-[11px] text-slate-400">
                        Supported media formats up to 10MB
                      </p>
                      <input
                    ...
                    {editing && !form[field.name] && field.read?.(editing) && (
                      <div className="mt-1 flex items-center gap-1.5 text-xs text-indigo-600">
                        <FileText size={13} />
                        <span>Existing file preserved unless overwritten.</span>
                      </div>
                    )}
                  </div>
                );
