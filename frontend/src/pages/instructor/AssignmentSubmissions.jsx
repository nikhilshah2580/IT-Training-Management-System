...
import { getAssignmentSubmissions, gradeSubmission } from "../../api/submission.services";
...
const AssignmentSubmissions = () => {
    ...
    const submitGrade = (submission) => {
        ...
        const payload = {
            grade: draft.grade !== undefined && draft.grade !== "" ? draft.grade : submission.grade,
            feedback: draft.feedback !== undefined ? draft.feedback : submission.feedback,
        };
    ...
    if (isError) {
        return (
            <div className="rounded-xl border bg-white p-8 text-center shadow-sm">
                <h2 className="text-xl font-semibold text-red-600">Failed to load assignment submissions</h2>
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    <Link to="/instructor/submissions" className="mb-3 inline-flex items-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-700">
                        <ArrowLeft size={16} />
                        Back to submissions
                    </Link>
                    <h1 className="text-2xl font-bold text-gray-900">{assignment?.title || "Assignment Submissions"}</h1>
                    <p className="mt-1 text-sm text-gray-500">
                        {assignment?.course?.title || "Course"} - {submissions.length} submission{submissions.length === 1 ? "" : "s"}
                    </p>
                </div>
                                    ...
                                    return (
                                        <tr key={submission._id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4">
                                                <p className="font-semibold text-gray-900">{submission.student?.fullName || "Unknown student"}</p>
                                                <p className="text-sm text-gray-500">{submission.student?.email || ""}</p>
                                            </td>
                                            <td className="px-6 py-4 text-sm text-gray-700">{formatDate(submission.submittedAt)}</td>
                                            <td className="px-6 py-4">
                                                <a href={submission.file} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-700">
                                            ...
                                            <td className="px-6 py-4">
                                                <input
                                                    ...
                                                    onChange={(event) => updateDraft(submission._id, "grade", event.target.value)}
                                            ...
                                            <td className="px-6 py-4">
                                                <textarea
                                                    ...
                                                    onChange={(event) => updateDraft(submission._id, "feedback", event.target.value)}
