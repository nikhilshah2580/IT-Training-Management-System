...
import { verifyOtp, forgotPassword } from "../../api/auth.services";
...
const VerifyOtp = () => {
  ...
  // If user navigated here directly without an email, send them back to forgot password
  useEffect(() => {
    if (!email) {
      showError("Please enter your email first", "Access Denied");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showError(error, "Invalid or expired OTP");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative rounded-lg shadow-sm">
              <input
                ...
                className="block w-full rounded-xl border border-slate-200 bg-white/60 px-3 py-3 text-center text-2xl font-bold tracking-widest text-slate-900 transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-indigo-600/10 sm:text-xl"
              />
            </div>
          </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-slate-500">
            ...
            <button
              ...
              disabled={resending}
              className="font-semibold text-indigo-600 hover:text-indigo-700 transition-colors disabled:opacity-50"
            >
