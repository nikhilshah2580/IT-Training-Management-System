...
const AttendanceManagement = () => {
  ...
  const students = readArray(studentsQuery.data, ["users"]).filter(
  ...
  return (
    <AdminModulePage
      ...
      description="Create, review, and update attendance records."
      queryKey="admin-attendance"
      listFn={() => getAttendance({ limit: 100 })}
      listKeys={["attendances", "attendance"]}
      createFn={markAttendance}
      updateFn={updateAttendance}
      deleteFn={deleteAttendance}
      createLabel="Mark Attendance"
      fields={[
        {
          name: "student",
          label: studentsQuery.isLoading
            ? "Loading students..."
            : "Select student",
          type: "select",
          required: true,
          options: students.map((student) => ({
            value: student._id,
            label: `${student.fullName || "Unnamed student"}${student.email ? ` (${student.email})` : ""}`,
          })),
          read: (row) => row.student?._id || row.student,
        },
        {
          name: "course",
      ...
      columns={[
        ...
        { label: "Date", render: (row, formatDate) => formatDate(row.date) },
