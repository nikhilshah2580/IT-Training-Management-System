...
const DemoClassManagement = () => (
  <AdminModulePage
    ...
    description="Review demo class requests and update their status."
    ...
    columns={[
      {
        label: "Student",
        render: (row) => row.student?.fullName || row.name || "-",
      },
      { label: "Course", render: (row) => row.course?.title || "-" },
      {
        label: "Schedule",
        render: (row, formatDate) => formatDate(row.scheduledAt || row.date),
      },
